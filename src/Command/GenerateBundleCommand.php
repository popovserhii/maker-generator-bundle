 ...
 *
 * @category Popov
 * @package Popov_MakerEnhancerBundle
...
use Popov\MakerGeneratorBundle\Generator\BundleGenerator;
use Popov\MakerGeneratorBundle\Manipulator\KernelManipulator;
use Popov\MakerGeneratorBundle\Manipulator\RoutingManipulator;
use Popov\MakerGeneratorBundle\Manipulator\ConfigurationManipulator;
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $runner($this->updateKernel($output, $this->kernel, $bundle));
        // routing importing
        ...
        // Instead of Command::SUCCESS we use 0 for BC with Symfony 4
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($askForBundleName) {
            ...
            $question->setValidator([Validators::class, 'validateBundleName']);
    ...
    protected function updateKernel(OutputInterface $output, KernelInterface $kernel, Bundle $bundle)
    {
        ...
        $output->writeln(sprintf(
            ...
            $this->makePathRelative($kernelManipulator->getBundlesConfigPath())
        ));
        try {
            ...
            if (!$ret) {
                ...
                return [
                    sprintf('- Edit <comment>%s</comment>', $kernelManipulator->getBundlesConfigPath()),
                    '  and add the following bundle in the <comment>return[]</comment> statement:',
                    '',
            ...
            return [
                sprintf('Bundle <comment>%s</comment> is already defined in <comment>%s</comment>.',
                    $bundle->getBundleClassName(),
                    $kernelManipulator->getBundlesConfigPath()
                ),
                '',
            ];
        }
    ...
    protected function updateRouting(OutputInterface $output, Bundle $bundle)
    {
        ...
        $routeName = RoutingManipulator::getImportedResourceYamlKey($bundle->getName(), '/');
        $targetRoutingPath = $this->params->get('kernel.project_dir') . sprintf('/config/routes/%s.yml', $routeName);
        $output->writeln(sprintf(
    ...
    protected function updateConfiguration(OutputInterface $output, Bundle $bundle)
    {
        ...
        $targetConfigurationPath = $this->params->get('kernel.project_dir') . '/config/config.yml';
        $output->writeln(sprintf(
    ...
    protected function createBundleObject(InputInterface $input)
    {
        ...
        $projectRootDirectory = $this->params->get('kernel.project_dir');
    ...
    {
        return new BundleGenerator($this->filesystem);
    }
}
